...
void BotUtilizePersonality(bot_state_t *bs)
{
	...
	if (len >= 131072)
	{
		...
		// GodOrDevil - 08.26.06 - 12:34am - #File handle - Close this beotch!
	...
	if (GetValueGroup(personalityBuffer, "BotWeaponWeights", group))
	{
		...
		for (i=0; i < WP_NUM_WEAPONS; i++)
		{
			if (GetPairedValue(group, bg_weaponNames[i], readbuf))
			{
				...
				// KRIS 9/04/2003 9:31PM
				//Com_Printf(S_COLOR_YELLOW"BotUtilizePersonality():- %s = %f\n", bg_weaponNames[i], bs->botWeaponWeights[i]);
				// KRIS
			}
		}
	}
