...
char *GetFlagStr( int flags )
{
	...
	flagstr = (char *)B_TempAlloc(MAX_FLAGSTR_SIZE);
	i = 0;
...
void BotWaypointRender(void)
{
	...
	while (i < gWPNum)
	{
		if (gWPArray[i] && gWPArray[i]->inuse)
		{
			...
			{
				plum = G_TempEntity( gWPArray[i]->origin, EV_BOTWAYPOINT );
				VectorCopy(gWPArray[i+1]->origin, plum->s.angles);
...
checkprint:
	...
	if (gotbestindex && bestindex != gLastPrintedIndex)
	{
		...
		//Don't bother at the moment. Number display is broken.
		*/
	}
...
{ //set waypoint weights depending on weapon and item placement
	...
	while (i < MAX_GENTITIES)
	{
		...
		if (ent && ent->classname)
		{
			...
			else if (strstr(ent->classname, "pickup_weapon_") && ent->item)
			{
...
int LoadPathData(const char *filename)
{
	...
	if (!f)
	{
		Com_Printf(S_COLOR_YELLOW "Bot route data not found\n");
		return 2;
	}
