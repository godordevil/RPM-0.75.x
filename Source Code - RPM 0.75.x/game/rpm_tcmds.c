...
void RPM_Tcmd ( gentity_t *ent )
{
	...
	// GodOrDevil - 09.18.06 - 04:57pm #compmode needed for tcmds no matter what
...
void RPM_TeamInfo (gentity_t *ent, char *team)
{
	...
	if(t == TEAM_RED && level.extras.redLocked)
	{
		trap_SendServerCommand( ent-g_entities, va("print \"^1Red ^3team is currently locked!\n\""));
	}

	else if( t == TEAM_BLUE && level.extras.blueLocked)
	{
...
{
	// GodOrDevil - 1.14.2005 - #compMode check - dont need
	///if (!g_compMode.integer)
	///{
	...
	if(referee)
	{
		if (team[0] == 'r' || team[0] == 'R')
		{
			if(level.extras.redLocked)
			{
				...
				// GodOrDevil - 09.18.06 - 04:47pm #lock,unlock
...
void RPM_Invite_Spec(gentity_t *ent, char *arg2)
{
	...
	if (ent->client->sess.team == TEAM_RED)
	{
		if(g_entities[id].client->sess.invitedByRed)
		{
			...
			trap_SendServerCommand( -1, va("print \"%s was Un-invited to spectate RED team.\n\"", g_entities[id].client->pers.netname ));
		...
		else
		{
			...
			trap_SendServerCommand( -1, va("print \"%s was invited to spectate RED team.\n\"", g_entities[id].client->pers.netname ));
	...
	{	
		...
		{
			//trap_SendServerCommand( ent-g_entities, va("print \"%s was Un-invited to spectate your team.\n\"", g_entities[id].client->pers.netname ));
			trap_SendServerCommand( -1, va("print \"%s was Un-invited to spectate BLUE team.\n\"", g_entities[id].client->pers.netname ));
		...
		{
			//trap_SendServerCommand( ent-g_entities, va("print \"%s was invited to spectate your team.\n\"", g_entities[id].client->pers.netname ));
			trap_SendServerCommand( -1, va("print \"%s was invited to spectate BLUE team.\n\"", g_entities[id].client->pers.netname ));
...
void RPM_Timeout(gentity_t *ent, qboolean referee)
{
	...
	if(level.extras.timeout || level.pause || level.intermissiontime ||  level.intermissionQueued)
	{
		return;
	}
	...
	{
		case TEAM_RED:
			if(level.extras.redtimeouts == g_timeoutLimit.integer)
			{
				trap_SendServerCommand( ent-g_entities, "print \"Your team has used all of its timeouts for this round.\n\"");
				return;
			}
			level.extras.timeout = TEAM_RED;
			level.extras.redtimeouts++;
			trap_SetConfigstring ( CS_GAMETYPE_MESSAGE, va("%i,@Timeout called by ^1Red ^7team", level.time + 10000 ) );
			RPM_Pause(ent);	
			//SC_adminLog(va("%s (%s) - TIMEOUT RED (REF)", ent->client->nvSess.cleanName, ent->client->nvSess.ip));
			NV_Log(6, va("%s (%s) - TIMEOUT RED (REF)", ent->client->nvSess.cleanName, ent->client->nvSess.ip));
			return;
		case TEAM_BLUE:
			if(level.extras.bluetimeouts == g_timeoutLimit.integer)
			{
				trap_SendServerCommand( ent-g_entities, "print \"Your team has used all of its timeouts for this round.\n\"");
				return;
			}
			level.extras.timeout = TEAM_BLUE;
			level.extras.bluetimeouts++;
			trap_SetConfigstring ( CS_GAMETYPE_MESSAGE, va("%i,@Timeout called by ^4Blue ^7team", level.time + 10000 ) );
			RPM_Pause(ent);
			//SC_adminLog(va("%s (%s) - TIMEOUT BLUE (REF)", ent->client->nvSess.cleanName, ent->client->nvSess.ip));
			NV_Log(6, va("%s (%s) - TIMEOUT BLUE (REF)", ent->client->nvSess.cleanName, ent->client->nvSess.ip));
			return;
		default:
			trap_SendServerCommand( ent-g_entities, "print \"Not on a valid Team!\n\"");
			return;
...
void RPM_Timein (gentity_t *ent)
{
	...
	if(level.extras.timeout == 3)
	{
		level.extras.timeout = 0;
		RPM_Unpause(ent);

		//SC_adminLog(va("%s (%s) - TIMEIN", ent->client->nvSess.cleanName, ent->client->nvSess.ip));
		NV_Log(6, va("%s (%s) - TIMEIN", ent->client->nvSess.cleanName, ent->client->nvSess.ip));
		return;
	}
	...
	if(level.extras.timeout != ent->client->sess.team)
	{
