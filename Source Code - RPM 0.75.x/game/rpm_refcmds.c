...
//	if (!Q_stricmp ( arg1, "?" ))
	...
	{
#ifndef _DEBUG
		trap_SendServerCommand( ent-g_entities, va("print \"^3Referee disabled for Competition!  Join Spec to use your powers.\n\""));
		return;
#endif
	}
	...
	// GodOrDevil - #Admin uses #ref commands - //03.20.05 - 01.40pm
	////Fixed the bug where non-admin refs couldnt use there powers.
	////if(!ent->client->nvSess.referee )
	if(!ent->client->nvSess.referee && !ent->client->nvSess.admin)
	...
	if (!Q_stricmp ( arg1, "chat" ))
	{
		...
		for(i = 0; i < level.maxclients; i++)
		{
			...
			if(tent->client->nvSess.admin || tent->client->nvSess.referee)
			{
				RPM_ClientSound(tent, G_SoundIndex("sound/misc/c4/beep.mp3"));
			}
		}
