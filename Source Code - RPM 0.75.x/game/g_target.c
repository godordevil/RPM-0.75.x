...
// GodOrDevil
//#bomb_kill
void bomb_kill_use( gentity_t *self, gentity_t *other, gentity_t *activator ) 
{
	G_PlayEffect( self->health, self->s.origin, self->pos1 );
 	G_RadiusDamage( self->r.currentOrigin, self, self->splashDamage, self->splashRadius, self, 1, HL_NONE );
}
...
{		
	...
	// if no effect file given just skip it
	if(G_SpawnString("effect", "", &effectName))
	{
		self->health = G_EffectIndex(effectName);
	}

	//G_SpawnInt ( "splashDamage", "0", &self->splashDamage );
	//G_SpawnInt ( "splashRadius", "0", &self->splashRadius );

	self->splashDamage = 1005;
	self->splashRadius = 100;

	//self->health = G_EffectIndex("effects/explosions/pra2_truck_explosion");
	self->use = bomb_kill_use;
}

//#RUSH - bomb use effect
void NV_Spawn_Target_Chunks ( gentity_t *ent );
void bomb_target_activated_use ( gentity_t* self )
{
	if(self->armTime >= gt_bombTimer.integer)
	{	
		//int effect;
		vec3_t dir; 
		...
		//effect = G_EffectIndex("explosions/mushroom_explosion.efx");
		//G_PlayEffect( effect, dir, self->pos1 );
		G_PlayEffect(level.effects[EFFECT_AIR_STRIKE_BIG], self->r.currentOrigin, self->s.angles);
		G_RadiusDamage( self->r.currentOrigin, self, self->splashDamage, self->splashRadius, self, 1, HL_NONE );
		self->armTime = 999999;

		NV_Sound(-1, "sound/misc/events/debris_splash.mp3");

		NV_Spawn_Target_Chunks ( self ); 

		//G_ClearGameTriggerConfigString();
		trap_UnlinkEntity (self);
		//self->inuse = qfalse;
		NV_checkGametypePhase();
...
#ifdef _DB_ENABLED
		if(self->activator != NULL && self->activator->client)
		{
			if(self->activator->client->pers.connected == CON_CONNECTED)
				self->activator->client->pers.statinfo.itemCaps++;
		}
#endif
	...
	//if( self->count % 2 == 1)
	//{
	//	NV_Sound(-1, "sound/ambience/generic/alarm07.wav");
	//}	
	
	if( self->armTime == 1)
	{
		NV_Sound(-1, "sound/misc/events/micro_beeps.mp3");
		//G_ClearGameTriggerConfigString();
		//G_SetGameTriggerConfigString();
	}
//	if( self->armTime == 2 || self->armTime % 4 == 1)
 	if( self->armTime % 2 == 1  || self->armTime % 3 == 1)
	{
		NV_Sound(-1, "sound/ambience/generic/alarm03.wav");
	}
	if( self->armTime % 20 == 1 )
	{
		vec3_t o;
		VectorCopy(self->s.origin, o);
		o[2] += 100;
		G_PlayEffect(G_EffectIndex ( "flare_blue.efx"), o, self->s.angles );
	}	
	else if( self->armTime % 10 == 1 )
	{
		vec3_t o;
		VectorCopy(self->s.origin, o);
		o[2] += 100;
		G_PlayEffect(G_EffectIndex ( "flare_red.efx"), o, self->s.angles );
	}
...
void bomb_target_red_use ( gentity_t *self, gentity_t *other, gentity_t *activator ) 
{
	...
	{
		//if(self->activator->client->pers.connected == CON_CONNECTED)
		self->activator->client->pers.statinfo.itemDefs++;
	}
#endif

	GT(va("%i,^7A ^3target ^7has been ^3Defused!", level.time + 5000));
}
void bomb_target_blue_use ( gentity_t *self, gentity_t *other, gentity_t *activator ) 
{
	...
	//G_PlayEffect( self->health, self->s.origin, self->pos1 );
 	//G_RadiusDamage( self->r.currentOrigin, self, self->splashDamage, self->splashRadius, self, 1, HL_NONE );
	self->use = bomb_target_red_use;
	self->think = bomb_target_activated_use;
	self->nextthink = level.time + 10;  // Let them all spawn first
}
//END
