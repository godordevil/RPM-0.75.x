...
static void PM_WalkMove( void ) {
	...
	// GodOrDevil - 05.22.06 - 04:17pm #Debug $FIX // GodOrDevil - 05.22.06 - 04:17pm 
...
void PM_CheckWeaponNotes ( void )
{
	...
	// GodOrDevil - 03.29.06 - 01:45am $FIX - Maybe caused by missing weapon anim or effect?
	///dll chokes on it.  Added #if
#if defined( Q3_VM ) 
	assert ( aW );
#endif
	///End  - 06.16.06 - 08:02pm
...
TAnimWeapon* PM_GetAnimFromName ( char *animName, playerState_t *ps, int *animIndex )
{
	...
	switch(ps->weapon)
	{
		...
		case WP_KNIFE:
			...
			{
				aW=BG_GetInviewAnimFromIndex(ps->weapon,ps->weaponAnimId&~ANIM_TOGGLEBIT);
				// GodOrDevil - bad shit here.. Need to check inview file or whatever to see
				/// ... what is fucking up!
				if(aW->mName == NULL){
 					aW=BG_GetInviewAnimFromIndex(ps->weapon,ps->weaponAnimId&~ANIM_TOGGLEBIT);
					Com_Printf("%i\n", ps->weaponAnimId&~ANIM_TOGGLEBIT);
					break;
				}
				///end
...
// GodOrDevil - Faster #Weapon Switch/Fire - DISABLE WHEN COMPILING CLIENT SIDE!!! 
////Works for server side but clients have a problem if the server isnt modded
////Created a cvar so it can be optional * use -D_WeaponTime to compile *
////For Server
#ifdef _WeaponTime
	switch( g_weapontime.integer)
	{
	case 1:
		pm->ps->weaponTime = min(150,pm->ps->weaponTime);
		break;
	case 2:
		pm->ps->weaponTime = min(200,pm->ps->weaponTime);
		break;
	case 3:
		pm->ps->weaponTime = min(250,pm->ps->weaponTime);
		break;	
	case 4:
		pm->ps->weaponTime = min(300,pm->ps->weaponTime);
		break;		
	case 5:
		pm->ps->weaponTime = min(350,pm->ps->weaponTime);
		break;		
	case 6:
		pm->ps->weaponTime = min(400,pm->ps->weaponTime);
		break;	
	case 7:
		pm->ps->weaponTime = min(450,pm->ps->weaponTime);
		break;	
	case 0:
	default:
...
int PM_GetAttackButtons(void)
{
	...
	// GodOrDevil
#ifdef QAGAME
	if ( pm->ps->weapon == WP_MSG90A1 || (weaponData[pm->ps->weapon].attack[ATTACK_ALTERNATE].weaponFlags & (1<<WP_FIREMODE_SINGLE)) || (g_m4scope.integer && pm->ps->weapon == WP_M4_ASSAULT_RIFLE) )
#else
...
static void PM_Weapon_AddInaccuracy( attackType_t attack )
{
	...
	if( (pm->ps->pm_flags & PMF_ZOOMED) && pml.groundPlane )
	{
		// GodOrDevil
#ifdef QAGAME
		if(!(pm->ps->weapon==WP_M4_ASSAULT_RIFLE && g_m4scope.integer != 1))
#endif
		//End
		return;
	}
...
static void PM_Weapon( void )
{
	...
	// GodOrDevil - 04.07.07 - 12:03am #MAPEDITOR - dont bother shooting if in editMode
#ifdef _MAPEDITOR
	if(pm->ps->pm_flags & PMF_MAPEDITOR)
		return;
#endif
	///End  - 04.07.07 - 12:03am
	...
	// GodOrDevil
#ifdef QAGAME
	if(pm->ps->weapon==WP_MSG90A1 || (g_m4scope.integer && pm->ps->weapon==WP_M4_ASSAULT_RIFLE ))
#else
	...
	// End
	{
		if( (attackButtons&BUTTON_ALT_ATTACK) || (pm->ps->pm_flags & PMF_ZOOM_REZOOM) )
		{
			...
			else
			{
				...
				// GodOrDevil
#ifdef QAGAME
				if(g_m4scope.integer == 2 && pm->ps->weapon==WP_M4_ASSAULT_RIFLE)
				{
					pm->ps->zoomFov = 20;
					pm->ps->weaponTime=425;
					pm->ps->weaponstate=WEAPON_ZOOMIN;
					PM_HandleWeaponAction(WACT_ZOOMIN);
				}
#endif
...
//			pm->cmd.buttons = pm->cmd.buttons & (BUTTON_RELOAD|BUTTON_ATTACK|BUTTON_);	
	...
	// GodOrDevil - 04.06.07 - 10:51pm #mapEditor
#ifdef _MAPEDITOR
	if ((pm->ps->pm_flags & PMF_MAPEDITOR) && ( pm->cmd.buttons & (BUTTON_USE | BUTTON_ATTACK | BUTTON_ALT_ATTACK)))
		return;
#endif
	///End  - 04.06.07 - 10:52pm
