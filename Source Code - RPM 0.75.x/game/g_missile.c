...
stop being SVF_BROADCAST and free up in 10 seconds
================
*/
void G_M15_SetFree(gentity_t *ent)
{
	ent->r.svFlags &= ~SVF_BROADCAST;
	ent->r.svFlags |= SVF_NOCLIENT;
	ent->nextthink = level.time + 9900;
	ent->think = G_FreeEntity;
}
// KRIS	


// GodOrDevil - #Gas #airstrike
...
EFFECT_FLARE_RED 2
EFFECT_FLARE_BLUE 3
EFFECT_GREEN_FLASH 4
EFFECT_MAX 10
*/
// GodOrDevil - #Gas
void NV_GAS_EFFECT(gentity_t *ent)
{	
	vec3_t dir;
	dir[0] = dir[1] = 0;
	dir[2] = 1;
	...
	G_PlayEffect(level.effects[EFFECT_GREEN_GAS], ent->r.currentOrigin, dir);
	ent->nextthink = level.time + 1000;				
}
// #AirStrike
void NV_AirStrike_EFFECT(gentity_t *ent)
{	
	vec3_t dir, origin;
	dir[0] = dir[1] = 0;
	dir[2] = 1;
	...
	if(ent->count == 1)
	{
		G_Sound(ent, CHAN_WEAPON, G_SoundIndex("sound/misc/events/f16_flyby.mp3"));
		ent->nextthink = level.time + 2500;
		return;
	}
	//if(ent->count >= 1 && ent->count <= 6 )
	//{
	//	origin[0]+= ent->count * 50;
	//	origin[2]+= ent->count * 50;
	//	G_PlayEffect(level.effects[4], ent->r.currentOrigin, dir);
	//	ent->nextthink = level.time + 1800 / ent->count;
	//	return;
	//}

	origin[0]+= irand(-160, 160);
	origin[1]+= irand(-160, 160);

	if(ent->count == 2 || ent->count == 6 || ent->count == 12)
	{
		G_PlayEffect(level.effects[EFFECT_AIR_STRIKE_BIG], origin, dir);
		G_CreateDamageArea ( origin, ent->parent, ent->splashDamage/2,ent->splashRadius, 300, ent->methodOfDeath );
	}
	else
	{
		G_PlayEffect(level.effects[EFFECT_AIR_STRIKE], origin, dir);
	}
	G_CreateDamageArea ( origin, ent->parent, ent->splashDamage/2,ent->splashRadius, 100, ent->methodOfDeath );
	G_RadiusDamage( origin,  ent->parent, ent->damage, ent->splashRadius, ent, 100, ent->splashMethodOfDeath );

	ent->nextthink = level.time + irand(200, 500);			
...
void G_ExplodeMissile( gentity_t *ent ) {
	...
	// KRIS 16/04/2003 1:42PM
	// check for blinding bots
#ifdef _CRS_BOTS
	if (ent->s.weapon == WP_M84_GRENADE)
	{
		Bot_FlashBang(origin);
	}
#endif
	// KRIS
	ent->s.eType = ET_GENERAL;
		...
		// GodOrDevil #GAS
	if(g_enableCustomGrenades.integer)
	{
#ifdef _SPECIAL_NADES
		if (ent->s.weapon == WP_F1_GRENADE)
		{		
			G_PlayEffect(level.effects[EFFECT_GREEN_FLASH], ent->r.currentOrigin, dir);
			G_PlayEffect(level.effects[EFFECT_GREEN_GAS], ent->r.currentOrigin, dir);

			//G_CreateDamageArea ( ent->r.currentOrigin, ent->parent, ent->splashDamage*0.05f,ent->splashRadius, 9500, ent->methodOfDeath );
			NV_Spawn_Gas (ent, ent->parent);
			ent->nextthink = level.time + 100;
			ent->think = NV_GAS_EFFECT;
			trap_LinkEntity( ent );
			return;
		}
#endif
		// #Airstrike
		if (ent->s.weapon == WP_MDN11_GRENADE)
		{		
			//G_PlayEffect(level.effects[4], ent->r.currentOrigin, dir);
			if(g_entities[1].client)
			{
				if(g_entities[ent->parent->s.number].client && g_entities[ent->parent->s.number].client->sess.team == TEAM_RED )
				{
					G_PlayEffect(level.effects[EFFECT_FLARE_RED], ent->r.currentOrigin, dir);
				}
				else 
				{
					G_PlayEffect(level.effects[EFFECT_FLARE_BLUE], ent->r.currentOrigin, dir);
				}
			}

			ent->nextthink = level.time + 3000;
			ent->think = NV_AirStrike_EFFECT;
			trap_LinkEntity( ent );
			return;
		}	
	}
	//if (ent->s.weapon == WP_L2A2_GRENADE)
	//{		
	//	//G_PlayEffect(level.effects[EFFECT_GREEN_FLASH], ent->r.currentOrigin, dir);
	//	ent->nextthink = level.time + 500;
	//	ent->think = NV_SHAKE_EFFECT;
	//	trap_LinkEntity( ent );
	//	return;
	//}	
	//	
	...
	// KRIS 16/04/2003 2:01PM
	if (ent->s.weapon == WP_M15_GRENADE)
	{
		ent->nextthink = level.time + 100;
		ent->think = G_M15_SetFree;
	}		
	else
	// KRIS
	...
	if (other->takedamage)
	{
		...
		if ( ent->damage )
		{
			...
			if ( d && other->client )
			{
				...
				if( &g_entities[ent->r.ownerNum] != other && (!level.gametypeData->teams || (level.gametypeData->teams && !OnSameTeam ( &g_entities[ent->r.ownerNum], other ))))
...
================
*/
#ifdef _MANDOWN_HITBOX // RPM added extra hitbox shit.. perhaps it lags the shit out of us?  Time to take it away
