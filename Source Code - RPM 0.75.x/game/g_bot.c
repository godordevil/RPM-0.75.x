...
{
	int			i;
	//char		netname[36];	// GodOrDevil - 09.09.06 - 11:55pm #REMOVED #kickBot
	gclient_t	*cl;
	...
	for ( i=0 ; i< g_maxclients.integer ; i++ ) 
	{
		...
		// GodOrDevil - 09.03.06 - 03:59pm #kickBot
...
int G_CountBotPlayers( int team ) 
{
	...
	for ( i=0 ; i< g_maxclients.integer ; i++ ) 
	{
		...
		//if ( team >= 0 && cl->sess.team != team ) 
		if ( team >= 0 && cl->sess.team != team || cl->sess.team == TEAM_SPECTATOR) 
		{
			continue;
		}
...
void G_CheckMinimumPlayers( void ) 
{
	...
	// KRIS 28/02/2003 5:15PM
	if (g_cheats.integer)
	{	// unless we're cheating, in which case PILE EM' IN QUICK!!
		if (checkminimumplayers_time > level.time - 300) 
		{
			return;
		}
	}
	...
	if ( level.gametypeData->teams ) 
	{
		...
		else if (botplayers && humanplayers + botplayers > minplayers) 
		{
...
#ifdef _NUMBERED_BOT_NAMES
	Info_SetValueForKey( userinfo, "name", va("bot%i^3[B]", irand(0,30000)));
#else
	Info_SetValueForKey( userinfo, "name", va("%s^3[B]", botname) );
#endif
...
static void G_LoadBots( void ) {
	...
	// KRIS 28/02/2003 5:18PM
	//numdirs = trap_FS_GetFileList("scripts", ".bot", dirlist, 1024 );
	numdirs = trap_FS_GetFileList("botfiles", ".txt", dirlist, 1024 );
	// KRIS
	dirptr  = dirlist;
	for (i = 0; i < numdirs; i++, dirptr += dirlen+1) {
		dirlen = strlen(dirptr);
		// KRIS 3/03/2003 8:59PM
...
#ifdef _CRS_BOTS	
	// KRIS 15/04/2003 11:38PM
	G_LoadPoints();
	// KRIS
#endif
}

// KRIS 15/04/2003 11:33PM
#ifdef _CRS_BOTS	
/*
==============
...
/*
===============
G_GetArenaInfoByNumber //Ryan LOL NUMBER?
===============
*/
...
===============
*/
// GodOrDevil - GAMETYPE - #map supports gametype
/*
...
qboolean G_DoesMapSupportGametype ( const char* gametype )
{
	...
	type = Info_ValueForKey( info, "longname" );
	trap_SetConfigstring( CS_MESSAGE, type );
