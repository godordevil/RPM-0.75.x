...
void G_MoverTeam( gentity_t *ent ) {
	...
	for ( part = ent ; part ; part = part->teamchain ) {
		...
		// GodOrDevil - 03.13.07 - 08:02pm #rotating
...
================
*/
// GodOrDevil - 03.16.07 - 02:14pm - #doors - from InVIMod
...
void SP_func_door (gentity_t *ent) {
	...
	// GodOrDevil - remove area portals & doors (weapon == 999 for SUB BSP in RMGs)
...
void Touch_Button(gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	// GodOrDevil - 03.14.07 - 02:04 pm #rotating
	///if ( ent->moverState == MOVER_POS1 ) {
	if ( ent->moverState == MOVER_POS1/* || ent->moverState == ROTATOR_POS1*/ ) {
	///End  - 03.14.07 - 02:04pm
...
void SP_func_wall ( gentity_t* ent )
{
	...
	// GodOrDevil - handle doors.
	//if (RMG.integer)
	if (RMG.integer || !g_enableDoors.integer)
	//END
	{
		trap_UnlinkEntity ( ent );
		return;
	...
	// GodOrDevil - #BOAT angle for liner3 - only checking the boat because.. well i dont want to test everything cuz i r lazy.	
	if(ent->targetname != NULL && !strcmp(ent->targetname, "boat"))
	{
		VectorCopy( ent->s.angles, ent->s.apos.trBase );
	}
	// END
...
void NV_ResetBreakable ( void )
{
	gentity_t* ent;
	ent = NULL;
	while ( NULL != (ent = G_Find ( ent, FOFS(classname), "blocker" ) ) )
	{		
		ent->damage = 0;
		trap_LinkEntity ( ent );
	}
...
void G_BreakableDie ( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod, int hitLocation, vec3_t hitDir )
{
	...
	// bomb sites are only effected by bombs
	if((self->spawnflags & 16) && strcmp("bomb_kill", attacker->classname))
		return;
	...
	G_RadiusDamage( self->r.currentOrigin, attacker, 5000, 1000, self, 10, MOD_TRIGGER_HURT );
	trap_UnlinkEntity(self);
}

void G_BreakableUse(gentity_t *self, gentity_t *other, gentity_t *activator)
{	
	if(other != NULL)
		G_BreakableDie(self, other, other, other->damage, MOD_UNKNOWN, HL_NONE, vec3_origin );
}
...
void SP_func_breakable( gentity_t *ent ) 
{
	...
	//ent->r.svFlags = SVF_BREAKABLE_BRUSH;
