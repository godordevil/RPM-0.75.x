...
// GodOrDevil - #spmaps
///Use this instead of "SP_misc_model" so we dont load 2 of each model.
void NV_model( gentity_t *ent ) 
{
	vec3_t	newAngles;

	ent->s.modelindex = G_ModelIndex( ent->model );
	VectorSet (ent->r.maxs, 16.0000, 16.0000, 16.0000);
	VectorSet (ent->r.mins, -16.000, -16.0000, 0.0000);

	// -- check for solid/non-solid value here so we can make beams or markers that arent solids
	...
	//MAKE THE BBOX SOLID
	//G_SpawnVector( "spawnflags", "", newAngles ));
	if(ent->spawnflags == 1)
	{
	...
	{
		ent->r.contents = CONTENTS_LADDER;
	}
	//END
	...
	if(G_SpawnVector( "angles", "0 0 0", newAngles )){
		...
		ent->s.angles[0] = newAngles[0];
		ent->s.angles[1] = newAngles[1];
		ent->s.angles[2] = newAngles[2];
	}
	if(G_SpawnVector( "mins", "-16 -16 0", newAngles )){
		SnapVector(newAngles);
		//VectorCopy(ent->r.maxs, maxs);
		ent->r.mins[0] = newAngles[0];
		ent->r.mins[1] = newAngles[1];
		ent->r.mins[2] = newAngles[2];
		ent->r.absmin[0] = newAngles[0];
		ent->r.absmin[1] = newAngles[1];
		ent->r.absmin[2] = newAngles[2];
	}
	if(G_SpawnVector( "maxs", "16 16 16", newAngles )){
		SnapVector(newAngles)
		//VectorCopy(ent->r.maxs, maxs);
		ent->r.maxs[0] = newAngles[0];
		ent->r.maxs[1] = newAngles[1];
		ent->r.maxs[2] = newAngles[2];
		ent->r.absmax[0] = newAngles[0];
		ent->r.absmax[1] = newAngles[1];
		ent->r.absmax[2] = newAngles[2];
	}
	G_SetOrigin( ent, ent->s.origin );
...
// GodOrDevil - 12.28.06 - 04:07pm #blockedTrigger
void NV_misc_bsp(gentity_t *ent) 
{
	...
	{
		ent->health = gt_targetHealth.integer * 1000;
	...
	//{
	//	ent->health = gt_targetHealth.integer;
	//	ent->moverState = MOVER_POS1;
	//	ent->takedamage = qtrue;
	//	ent->die = G_BreakableDie;
	//	return;
	...
	////end
#ifdef _FORT
	//G_SpawnInt( "fort_item", "0", &tempint);
	//if(tempint)
	//{
	//	//ent->moverState = MOVER_POS1;	
	//	//ent->s.eType = ET_INVISIBLE;
	//	ent->clipmask = MASK_WATER;
	//	ent->r.contents = MASK_WATER;
	//}
...
//	shaderNum = gi.CM_GetShaderNum(s.modelindex);
	...
	if (RMG.integer)
	{
		...
		// GodOrDevil #RMG TEST - set this here so it wont pick whatever it wants and fuck up the terrain.
		//trap_Cvar_VariableStringBuffer("RMG_mission", missionType, MAX_QPATH);
		trap_Cvar_Set("RMG_mission", NV_getBasedGameType(g_gametype.string));
		trap_Cvar_VariableStringBuffer("RMG_mission", missionType, MAX_QPATH);
		//END
...
void SP_fx_play_effect(gentity_t *ent)
{
	...
	if(!G_SpawnString("effect", "", &fxName))
	{
		G_SpawnString("message2", "", &fxName);
	}
...
//	{
//		G_PlayEffect(G_EffectIndex ( fxName), ent->s.origin, ent->pos1 );
//	}
//#endif
}
