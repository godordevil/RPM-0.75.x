...
void NV_saveShaders(gentity_t *ent) 
{
	...
	if (trap_FS_FOpenFile( va("%s%s_%s.cfg", altDir, mapname, nv_gtShort.string), &f, FS_WRITE ) < 0) {
		CPe(va("cp \"^3Cannot open file!\n"));
		CPe(va("cp \"^3SHADERS WILL ^1NOT ^3BE ^1SAVED^3!\n"));
		return;
	}
	for (i = 0; i < remapCount; i++) {
		Com_sprintf(out, (MAX_QPATH * 2) + 5, "%s %s\n", remappedShaders[i].oldShader, remappedShaders[i].newShader);
		trap_FS_Write( out, strlen( out ), f );
	}	
	trap_FS_FCloseFile( f );
	CPe(va("cp \"^3Save Successful: ^7%s%s_%s.cfg\n", altDir, mapname, nv_gtShort.string));
}
...
void NV_showShaders (gentity_t *ent) 
{
	...
	CPe(va("print \"^3 %i SHADERS REMAPPED:\n^7------------------------\n", remapCount));

	for (i = 0; i < remapCount; i++)
	{
		CPe(va("print \"^3[%2i]  ^1%s\n      ^5%s\n", i+1, remappedShaders[i].oldShader, remappedShaders[i].newShader));
	}	
}
#endif
// END
...
int G_FindConfigstringIndex( char *name, int start, int max, qboolean create ) 
{
	...
	// GodOrDevil - 09.01.06 - 12:30am #FIX
	/// Debugger barfs 
#ifdef _DEBUG
	if ( !name || name[0] == 0) 
		return 0;
...
//		Com_Error( ERR_FATAL, "G_FindConfigstringIndex: overflow" );
		return 0;			/// ROCmod - GodOrDevil - 01.27.06 - 09:51pm
	}
	// GodOrDevil - keep track of unique #bsp count
	if(start == CS_BSP_MODELS)
		uniqueBSPS++;
	///End
...
gentity_t* G_Spawn( void ) 
{
...
#ifdef _CLEAR_DROPPED
			//if ( inUse >= ENTITYNUM_MAX_NORMAL - 200)
		//	if ( inUse >= 600)
			if ( dropped > 10 && inUse >= 580)
 				NV_clearDroppedItems();
#endif // 120317
	...
	{
#ifdef _DEBUG
		NV_Log (2, va("\n\nNum Entities: %i (max normal: %i)\n-----------------", level.num_entities, i)) ;
#endif
		for (i = 0; i < MAX_GENTITIES; i++) 
		{
			Com_Printf("%4i: %s\n", i, g_entities[i].classname);
#ifdef _DEBUG
			NV_Log (2, va("[%4i] %s", i,  g_entities[i].classname)) ;
#endif
		}
...
// GodOrDevil - 02.04.05 - 09:14am #GOLD #FreeEntity
/*
=================
