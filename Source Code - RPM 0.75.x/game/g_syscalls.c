...
// GodOrDevil - 10.23.05 - 03:09pm - #crashFix
/// ,,, this file is for dll only. Testing purposes only
/// ,,, Check nv_crashFix.c for qvm fix!
...
{
#ifdef _DB_ENABLED
	if(exec_when >= DB_LITE_INIT_NEW && !g_enableDatabase.integer)
		return;
#endif
	if ( strlen( text ) >= 1022 )
	{
		char a[1022]; 
		Q_strncpyz (a, text, 1022);
		syscall( G_SEND_CONSOLE_COMMAND, exec_when, a );
		a[800] = '\0';
		Com_Printf("Warning: Oversized command attempt.\n");
		NV_crashLog (va("overflow: %s", a )) ;
		return;
	}
	syscall( G_SEND_CONSOLE_COMMAND, exec_when, text );
}
...
void trap_SendServerCommand( int clientNum, const char *text ) {
	// GodOrDevil - 04.30.06 - 03:08pm - #crashFix
	if ( strlen( text ) > 1022 ) {
		char a[1022]; 
		Q_strncpyz (a, text, 700);
		Com_Printf("Warning: Oversized command attempt to client %i.\n", clientNum);
		trap_SendServerCommand(clientNum, va("print \"Oversized command from server\n\"") );
		NV_crashLog (va("overflow servercmd: name:[%s] - size:[%i]", g_entities[clientNum].client->pers.netname, strlen( text ) )) ;
		syscall( G_SEND_SERVER_COMMAND, clientNum, a );
		return;
	}
	///End  - 04.30.06 - 03:10pm
...
#ifdef _DB_ENABLED
//void trap_QUERY_Update(int type, void *extra, gentity_t *gEnts,	gclient_t *client, level_locals_t *levels, statinfo_t *stats, dbConnectInfo_t *db)
//{
	//syscall( NV_QUERY_CALL, type, extra, gEnts, client, levels, stats, db);
//}
#endif
