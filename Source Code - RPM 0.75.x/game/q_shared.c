...
void Q_strcat( char *dest, int size, const char *src ) {
	...
	l1 = strlen( dest );
	if ( l1 >= size ) {
// GodOrDevil - 01.24.06 - 08:11pm
//		Com_Error( ERR_FATAL, "Q_strcat: already overflowed" );
		Com_Printf( "Q_strcat: already overflowed" );
		return;			// ROCmod
//End  - 01.24.06 - 08:11pm
	}
...
int Q_PrintStrlen( const char *string ) {
	...
	len = 0;
	p = string;
// GodOrDevil - 01.24.06 - 08:16pm #gold #TEST
	while( *p ) 
	{
		if( *p == '^' ) 
		{
...
// GodOrDevil - 01.24.06 - 08:12pm
	// ROCmod - If we get an overflow, just quit
	if ( strlen( s ) >= BIG_INFO_STRING || strlen( key ) >= BIG_INFO_KEY ) {
...
// GodOrDevil - 01.24.06 - 08:13pm
	// ROCmod - If we get an overflow, just quit
	if ( strlen( s ) >= MAX_INFO_STRING || strlen( key ) >= BIG_INFO_KEY ) {
//		Com_Error( ERR_DROP, "Info_RemoveKey: oversize infostring" );
		Com_Printf( "Info_RemoveKey: oversize infostring" );
		return;
//End  - 01.24.06 - 08:13pm
...
//		Com_Error( ERR_DROP, "Info_RemoveKey_Big: oversize infostring" );
		Com_Printf( "Info_RemoveKey_Big: oversize infostring" );
		return;
	}
//End  - 01.24.06 - 08:13pm
	if (strchr (key, '\\')) {
...
// GodOrDevil - 01.24.06 - 08:14pm
	...
	if (strlen(newi) + strlen(s) >= MAX_INFO_STRING)
	{
		Com_Printf ("Info string length exceeded: '%s' + '%s'\n", newi, s);
