...
void G_TraceBullet ( weapon_t weapon, trace_t* tr, G2Trace_t G2Trace, vec3_t start, vec3_t end, int passent, int mask, qboolean detailed )
{
	...
	// GodOrDevil - #TFT - can cut corpses
	if(weapon == WP_KNIFE && (NV_isTFT() || NV_isFT()))
	{
		mask |= CONTENTS_CORPSE;
	}
	//END
...
*/	// KRIS
	...
	// KRIS 7/08/2003 11:07AM
		...
		// GodOrDevil - #TFT - can cut corpses
		if(weapon == WP_AK74_ASSAULT_RIFLE && attack == ATTACK_ALTERNATE && (NV_isTFT() || NV_isFT()))
		{
			G_TraceBullet ( (weapon_t)weapon, &tr, G2Trace, muzzlePoint, end, ent->s.number, MASK_SHOT | CONTENTS_CORPSE, detailed );
		}
		else
		//END
		...
		if ( traceEnt->takedamage ) 
		{
			...
			if ( (int)(attackDat->damage * damageMult) > 0 )
			{
				...
				//Ryan april 7 2003
				//set which weapon was used (for RPM_Obituary)
				stat->weapon = weapon; 
				...
				if (!level.gametypeData->teams || !OnSameTeam(ent,traceEnt))
				{
					...
					switch ( location )
					{
						...
						case HL_FOOT_LT:
							...
							break;
						case HL_HAND_RT:
		// GodOrDevil - Shot in hand while holding nade == instant detonation!
#ifdef _DEBUG 
							/* testing
							if(traceEnt->client->ps.weapon)
							{
								traceEnt->client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_M67_GRENADE );
								traceEnt->client->ps.weapon = WP_M67_GRENADE;
								traceEnt->client->ps.weaponstate=WEAPON_CHARGING_ALT;
								traceEnt->client->ps.grenadeTimer = 1;
								Com_Printf(S_COLOR_YELLOW"G_FireBullet():- Shot Grenade hand\n");
							}
							*/
							if(traceEnt->client->ps.weapon >= WP_M67_GRENADE)
							{
								traceEnt->client->ps.weaponstate=WEAPON_CHARGING_ALT;
								traceEnt->client->ps.grenadeTimer = 1;
							}
	...
	if ( hitcount )
	{
		...
		{	
			...
			//	G_Damage( hit[h].ent, ent, ent, fwd, hit[h].origin, 10000, flags, attackDat->mod + (attack<<8), hit[h].location );
			//else
...
gentity_t* G_FireProjectile ( gentity_t *ent, weapon_t weapon, attackType_t attack, int projectileLifetime, int flags )
{
	...
	// GodOrDevil - Only if FF is on!  Fucking annoying
