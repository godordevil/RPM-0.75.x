...
static cvarTable_t gametypeCvarTable[] = 
{
	...
	{ &gt_bombFuseTime,		"gt_bombFuseTime",		"45", CVAR_ARCHIVE|CVAR_LATCH, 0.0f, 0.0f, 0, qtrue },
	...
	{ &gt_bombsNeeded,		"gt_bombsNeeded",		"3",  CVAR_ARCHIVE|CVAR_LATCH, 0.0f, 0.0f, 0, qtrue },
...
void GT_RegisterCvars( void ) 
{
	...
	for ( cv = gametypeCvarTable ; cv->cvarName != NULL; cv++ ) 
	{
		...
		//RCW - check for a cvar change and call the GT_INIT again if needed (register GT specific shit)
		//if (!Q_stricmp(cv->cvarName,"gt_customType") && cv->vmCvar->modificationCount < 1)
