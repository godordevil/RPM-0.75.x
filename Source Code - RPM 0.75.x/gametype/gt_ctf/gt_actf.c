	...
	switch ( cmd )
	{
		...
		case GTEV_ITEM_STUCK:
			switch ( arg0 )
			{
				case ITEM_REDFLAG:
					trap_Cmd_ResetItem ( ITEM_REDFLAG );
					trap_Cmd_TextMessage ( -1, "^7The ^1Red ^7Flag has returned!" );
					trap_Cmd_StartGlobalSound ( gametype.flagReturnSound );
					gametype.redFlagDropTime = 0;
					return 1;
				...
				case ITEM_BLUEFLAG:
					trap_Cmd_ResetItem ( ITEM_BLUEFLAG );
					trap_Cmd_TextMessage ( -1, "^7The ^4Blue ^7Flag has returned!" );
					trap_Cmd_StartGlobalSound ( gametype.flagReturnSound );
					gametype.blueFlagDropTime = 0;
					return 1;
			}
		...
		case GTEV_ITEM_TOUCHED:
			...
			switch ( arg0 )
			{
				case ITEM_REDFLAG:
				case ITEM_BLUEFLAG:
					if ( arg2 == TEAM_RED || arg2 == TEAM_BLUE )
					{
						char clientname[MAX_QPATH];
						trap_Cmd_GetClientName ( arg1, clientname, MAX_QPATH );
						switch ( arg2 )
						{
						case TEAM_BLUE:
							trap_Cmd_TextMessage ( -1, va("^3%s ^7has taken the ^4Flag^7!", clientname ) );
							break;
						case TEAM_RED:
							trap_Cmd_TextMessage ( -1, va("^3%s ^7has taken the ^1Flag^7!", clientname ) );
							break;
						}
