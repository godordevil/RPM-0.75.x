...
void UI_LoadArenas( void ) 
{
	...
	for( n = 0; n < ui_numArenas; n++ ) 
	{
		...
		if( *type ) 
		{
			...
			{
				// Ryan Dec 5 2004
				// Add a check for the base gametype so the map shows in the 
				// create server map list if it supports the selected gametype's base gametype
				//if ( strstr ( type, bg_gametypeData[gametype].name ) )
				if ( strstr ( type, bg_gametypeData[gametype].name ) || strstr ( type, bg_gametypeData[gametype].basegametype))
				// Ryan
				{
