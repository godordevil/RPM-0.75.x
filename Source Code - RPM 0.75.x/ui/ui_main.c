...
//Ryan march 31 2004 9:16pm
#include "rpm_ui_public.h"
//Ryan
...
void UI_LoadMenus ( const char *menuFile, qboolean reset ) 
{
	...
	//Ryan May 5 2004
	//This was added to keep the client-side mod from interfering with other mods like radio
	//mods etc.. that contain ingame.txt files
	//So ill just do a recursive function call to load the rpm files at a different time
	//Also note we do not reset the menu or else we loose the whole ingame menu
	if(!Q_stricmp(menuFile, "ui/ingame.txt"))
	{
		UI_LoadMenus( "ui/rpm_ingame.txt", qfalse);
	}
	//Ryan
}
...
static void UI_LoadBsps(qboolean maps) {
	char			movielist[16096];
	char			*moviename;
	unsigned int	i, len;
	unsigned int	count;	

	///uiInfo.bspCount = trap_FS_GetFileList( "maps/instances", ".bsp", movielist, sizeof(movielist) );
	//uiInfo.bspCount = trap_FS_GetFileList( "maps/instances", ".bsp", movielist, sizeof(movielist) );
	if(maps)
	//	uiInfo.bspCount = trap_FS_GetFileList( "maps/", "bsp", movielist, sizeof(movielist) );
		//uiInfo.bspCount = trap_FS_GetFileList( "$modlist/../../", "", movielist, sizeof(movielist) );
		//uiInfo.bspCount = trap_FS_GetFileList( "$modlist/../../../../", "", movielist, sizeof(movielist) );
		//uiInfo.bspCount = trap_FS_GetFileList( "/../../../../..", "/", movielist, sizeof(movielist) ); // wow.. just wow!
		//uiInfo.bspCount = trap_FS_GetFileList( "/../../../", "/", movielist, sizeof(movielist) );
		//uiInfo.bspCount = trap_FS_GetFileList( "$modlist/../../", "", movielist, sizeof(movielist) );// List of files to possibly check?
		uiInfo.bspCount = trap_FS_GetFileList( "maps/", ".bsp", movielist, sizeof(movielist) );// List of files to possibly check?
	else
		uiInfo.bspCount = trap_FS_GetFileList( "maps/instances", ".bsp", movielist, sizeof(movielist) );

	if (uiInfo.bspCount) 
	{
		if (uiInfo.bspCount > MAX_MOVIES) 
		{
			uiInfo.bspCount = MAX_MOVIES;
		}
		moviename = movielist;
		for ( i = 0, count = 0; i < uiInfo.bspCount; i++ ) 
		{
			len = strlen( moviename );
			if(maps)
			{	
				char* dir;
				dir = strchr ( moviename, '/' );
				if ( dir )
				{
					moviename += len + 1;
					continue;
			...
			if (Q_stricmp(moviename +  len - 10,"boat01.bsp")) 
			{
				if (!Q_stricmp(moviename +  len - 4,".bsp")) 
				{
					moviename[len-4] = '\0';
				}
				//Q_strupr(moviename);
				if(!maps)
				{
					uiInfo.bspList[count++] = String_Alloc(va("INSTANCES/%s", moviename));
				}
				else
				{
					uiInfo.bspList[count++] = String_Alloc(moviename);

				}
			}
			moviename += len + 1;
		}
		uiInfo.bspCount = count;
	}
}

static void UI_LoadModels(qboolean objects)
{
	//char			list[31096];
	char			list[31096];
	char			*name;
	...
	//uiInfo.modelCount = trap_FS_GetFileList( "/", ".md3", list, sizeof(list) );
	//uiInfo.modelCount = trap_FS_GetFileList( "models/", ".md3", list, sizeof(list) );
	//uiInfo.modelCount = trap_FS_GetFileList( "models/", ".md3", list, sizeof(list) );
	//uiInfo.modelCount = trap_FS_GetFileList( "", ".md3", list, sizeof(list) );
	if(objects)
		uiInfo.modelCount = trap_FS_GetFileList( "models/objects/", ".md3", list, sizeof(list) );
	else
		uiInfo.modelCount = trap_FS_GetFileList( "models/", ".md3", list, sizeof(list) );

	if (uiInfo.modelCount) 
	{
		if (uiInfo.modelCount > MAX_MODEL_LIST) 
		{
			uiInfo.modelCount = MAX_MODEL_LIST;
		}
		name = list;
		for ( i = 0, count = 0; i < uiInfo.modelCount; i++ ) 
		{
			len = strlen( name );
			if (!Q_stricmp(name +  len - 4,".md3")) 
			{
				name[len-4] = '\0';
			}
			//Q_strupr(name);
			if(objects)
			{
				uiInfo.modelList[count++] = String_Alloc(va("objects/%s", name));
			}
			else
			{
				if(Q_strncmp (name, "objects/", 8))
				{
					uiInfo.modelList[count++] = String_Alloc(name);
				}
			}
			//uiInfo.modelList[i] = String_Alloc(name);
			name += len + 1;
		}
		uiInfo.modelCount = count;
	}
}
...
static void UI_RunMenuScript(const char **args) 
{
	...
	{		
...
#ifndef _DEBUG // GodOrDevil - keeps crashing my client DLL errrr
		...
		// GodOrDevil
		} else if (Q_stricmp(name, "LoadBsps") == 0) {
			UI_LoadBsps(qfalse);		
		} else if (Q_stricmp(name, "LoadBspsMaps") == 0) {
			UI_LoadBsps(qtrue);		
		} else if (Q_stricmp(name, "LoadModels") == 0) {
			UI_LoadModels(qtrue);
		} else if (Q_stricmp(name, "LoadModelsMisc") == 0) {
			UI_LoadModels(qfalse);
		} else if (Q_stricmp(name, "RunBsp") == 0) {
			trap_Cmd_ExecuteText( EXEC_APPEND, va("additem bsp %s;", uiInfo.bspList[uiInfo.bspIndex]) );
		} else if (Q_stricmp(name, "RunModel") == 0) {
			trap_Cmd_ExecuteText( EXEC_APPEND, va("additem model models/%s.md3;", uiInfo.modelList[uiInfo.modelIndex]) );
		// END
		...
		//Ryan Dec 5 2003
		else if (Q_stricmp(name, "voteRef") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
		...
		else if (Q_stricmp(name, "voteUnRef") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
		...
		else if (Q_stricmp(name, "playerStats") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("stats %s\n",clientnum) );
			}
		} 
	/*	else if (Q_stricmp(name, "admkick") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("adm kick %s\n",clientnum) );
			}
		} 
		else if (Q_stricmp(name, "admban") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("adm ban %s\n",clientnum) );
			}
		} 
		else if (Q_stricmp(name, "admpop") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("adm pop %s\n",clientnum) );
			}
		} 
		else if (Q_stricmp(name, "admmute") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("adm mute %s\n",clientnum) );
			}
		} 
		else if (Q_stricmp(name, "admunmute") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("adm unmute %s\n",clientnum) );
			}
		} 
		else if (Q_stricmp(name, "admrunover") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("adm runover %s\n",clientnum) );
			}
		} 
		else if (Q_stricmp(name, "admstrip") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("adm strip %s\n",clientnum) );
			}
		}
		else if (Q_stricmp(name, "admplant") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("adm plant %s\n",clientnum) );
			}
		} 
		else if (Q_stricmp(name, "admplantsk") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("adm plantsk %s\n",clientnum) );
			}
		} 
*/
		else if (Q_stricmp(name, "invitespec") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("tcmd invite %s\n",clientnum) );
			}
		} 
		else if (Q_stricmp(name, "uninvitespec") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("tcmd uninvite %s\n",clientnum) );
			}
		} 
		else if (Q_stricmp(name, "refkick") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("ref kick %s \"%s\"\n", clientnum, ui_clientKickReason.string) );
			}
		} 
		//Ryan

		//Ryan march 7 2004 9:31pm
		else if(Q_stricmp(name, "loadSongs") == 0)
		{
			UI_LoadSongs();
		}

		else if (Q_stricmp(name, "changeSong") == 0) 
		{		
			trap_Cmd_ExecuteText( EXEC_APPEND, va("song \"%s\"\n",uiInfo.songList[uiInfo.songIndex]) );	
		} 
		//Ryan

		//Ryan march 31 2004 8:46pm
		else if (Q_stricmp(name, "playerPunish") == 0) 
		{
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) 
			{
				char clientnum[3];
				...
				trap_Cmd_ExecuteText( EXEC_APPEND, va("adm %s %s \"%s\"\n", punishCmds[ui_admPunishCmd.integer], clientnum, ui_clientKickReason.string ));
			}
		}
		//Ryan

		//Ryan april 2 2004
		else if (Q_stricmp(name, "clearAdmPunishData") == 0)
		{
			trap_Cvar_Set( "ui_admPunishCmd", "0");
			trap_Cvar_Set( "ui_clientKickReason", "");
			trap_Cvar_Set( "ui_authMessage", "");
			Menu_SetFeederSelection(NULL, FEEDER_PLAYER_PUNISHMENTS, 0, "rpm_Admin_Cmds");
			Menu_SetFeederSelection(NULL, FEEDER_PLAYER_LIST, 0, "rpm_Admin_Cmds");
		}
		else if (Q_stricmp(name, "admAllMessage") == 0)
		{
			if(ui_authMessage.string[0])
			{
				trap_Cmd_ExecuteText( EXEC_APPEND, va("adm talk \"%s\"\n", ui_authMessage.string));
			}
		}
		else if (Q_stricmp(name, "admOnlyMessage") == 0)
		{
			if(ui_authMessage.string[0])
			{
				trap_Cmd_ExecuteText( EXEC_APPEND, va("adm chat \"%s\"\n", ui_authMessage.string));
			}
		}
		//Ryan

		//Ryan april 3 2004
		else if (Q_stricmp(name, "refTeamCommand") == 0)
		{
			trap_Cmd_ExecuteText( EXEC_APPEND, va("ref %s %s\n", refTeamCmd[refTeamCmdIndex], teamList[refTeamIndex] ));
			if(!refTeamCmdIndex)
			{
				trap_Cmd_ExecuteText( EXEC_APPEND, "toggleConsole\n");
			}
		}
		else if (Q_stricmp(name, "clearRefCmdData") == 0)
		{
			refTeamCmdIndex = 0;
			refTeamIndex = 0;
			trap_Cvar_Set( "ui_clientKickReason", "");
			trap_Cvar_Set( "ui_authMessage", "");
			Menu_SetFeederSelection(NULL, FEEDER_REF_TEAM_COMMANDS, 0, "rpm_Ref_Cmds");
			Menu_SetFeederSelection(NULL, FEEDER_TEAM_LIST, 0, "rpm_Ref_Cmds");
		}
		else if (Q_stricmp(name, "refAllMessage") == 0)
		{
			if(ui_authMessage.string[0])
			{
				trap_Cmd_ExecuteText( EXEC_APPEND, va("ref talk \"%s\"\n", ui_authMessage.string));
			}
		}
		else if (Q_stricmp(name, "refOnlyMessage") == 0)
		{
			if(ui_authMessage.string[0])
			{
				trap_Cmd_ExecuteText( EXEC_APPEND, va("ref chat \"%s\"\n", ui_authMessage.string));
			}
		}
		else if (Q_stricmp(name, "rpmvote") == 0)
		{
			char clientnum[3] = "\0";
...
static int UI_FeederCount(float feederID) 
{
	switch ( (int)feederID )
	{
		...
		//Ryan march 7 2004 8:02pm
		case FEEDER_SONG_LIST:
			return uiInfo.songCount;
		case FEEDER_PLAYER_PUNISHMENTS:
			return PUNISH_COUNT;
		case FEEDER_REF_TEAM_COMMANDS:
			return REF_TEAM_COMMAND_COUNT;
		...
		// GodOrDevil #feeders
		case FEEDER_BSP:
			return uiInfo.bspCount;
		case FEEDER_MODELS:
			return uiInfo.modelCount;
		// END
	...
	*handle = -1;
	...
	switch ( (int)feederID )
	{
		...
		case FEEDER_DEMOS:
			...
			break;
		//Ryan march 7 2004 9:07pm
		case FEEDER_SONG_LIST:
			if (index >= 0 && index < uiInfo.songCount) 
			{
				return uiInfo.songList[index];
			}
			break;

		case FEEDER_PLAYER_PUNISHMENTS:
			if (index >= 0 && index < PUNISH_COUNT) 
			{
				return punishList[index];
			}
			break;
		case FEEDER_REF_TEAM_COMMANDS:
			if (index >= 0 && index < REF_TEAM_COMMAND_COUNT) 
			{
				return refTeamCmdList[index];
			}
			break;
		case FEEDER_TEAM_LIST:
			if (index >= 0 && index < 2) 
			...
			break;
		case FEEDER_VOTES:
			if(index >= 0 && index < VOTECOUNT)
			{
				return voteList[index][0];
			}
			break;
		//Ryan
		...
		// GodOrDevil 
		case FEEDER_BSP:
			if (index >= 0 && index < uiInfo.bspCount) 
			{
				return uiInfo.bspList[index];
			}
			break;
		case FEEDER_MODELS:
			if (index >= 0 && index < uiInfo.modelCount) 
			{
				return uiInfo.modelList[index];
			}
			break;
		// END
	}
...
static void UI_FeederSelection(float feederID, int index) 
{
	...
	switch ( (int)feederID )
	{
		...
		// GodOrDevil #feeders
		case FEEDER_BSP:
			uiInfo.bspIndex = index;
			break;
		case FEEDER_MODELS:
			uiInfo.modelIndex = index;
			break;
		// END

		//Ryan march 7 2004 9:25pm
		case FEEDER_SONG_LIST:
			uiInfo.songIndex = index;
			break;
		case FEEDER_PLAYER_PUNISHMENTS:
			trap_Cvar_Set( "ui_admPunishCmd", va("%d", index));
			break;
		case FEEDER_REF_TEAM_COMMANDS:
			refTeamCmdIndex = index;
			break;
		case FEEDER_TEAM_LIST:
			refTeamIndex = index;
			break;
		case FEEDER_VOTES:
			trap_Cvar_Set( "ui_voteIndex", va("%d", index));
			break;
		//Ryan
	}
}
...
//			trap_Cvar_Set( "cl_paused", "1" );
		...
		case UIMENU_MAPEDITOR:
			trap_Key_SetCatcher( KEYCATCH_UI );
			Menus_CloseAll();
			trap_Cvar_Set( "cl_paused", "1" );
			Menus_ActivateByName("ingame_mapEditor");
			return;
		// END
	}
}
...
static void UI_DisplayDownloadInfo( const char *downloadName, float centerPoint, float yStart, float scale ) 
{
	...
	Text_PaintCenter(centerPoint, yStart + 112, uiInfo.uiDC.Assets.defaultFont, scale, colorWhite, dlText, 0 );
	Text_PaintCenter(centerPoint, yStart + 192, uiInfo.uiDC.Assets.defaultFont, scale, colorWhite, etaText, 0 );
	Text_PaintCenter(centerPoint, yStart + 248, uiInfo.uiDC.Assets.defaultFont, scale, colorWhite, xferText, 0 );
	...
	Text_PaintCenter(centerPoint, yStart+136, uiInfo.uiDC.Assets.defaultFont, scale, colorWhite, s, 0 );

	UI_ReadableSize( dlSizeBuf,		sizeof dlSizeBuf,		downloadCount );
	UI_ReadableSize( totalSizeBuf,	sizeof totalSizeBuf,	downloadSize );

	if (downloadCount < 4096 || !downloadTime) {
		Text_PaintCenter(leftWidth, yStart+216, uiInfo.uiDC.Assets.defaultFont, scale, colorWhite, "estimating", 0 );
		Text_PaintCenter(leftWidth, yStart+160, uiInfo.uiDC.Assets.defaultFont, scale, colorWhite, va("(%s of %s copied)", dlSizeBuf, totalSizeBuf), 0 );
	} else {
		...
		if (downloadSize && xferRate) {
			...
			Text_PaintCenter(leftWidth, yStart+216, uiInfo.uiDC.Assets.defaultFont, scale, colorWhite, dlTimeBuf, 0 );
			Text_PaintCenter(leftWidth, yStart+160, uiInfo.uiDC.Assets.defaultFont, scale, colorWhite, va("(%s of %s copied)", dlSizeBuf, totalSizeBuf), 0 );
		} else {
			Text_PaintCenter(leftWidth, yStart+216, uiInfo.uiDC.Assets.defaultFont, scale, colorWhite, "estimating", 0 );
			if (downloadSize) {
				...
				Text_PaintCenter(leftWidth, yStart+160, uiInfo.uiDC.Assets.defaultFont, scale, colorWhite, va("(%s copied)", dlSizeBuf), 0 );
			}
		}

		if (xferRate) {
			Text_PaintCenter(leftWidth, yStart+272, uiInfo.uiDC.Assets.defaultFont, scale, colorWhite, va("%s/Sec", xferRateBuf), 0 );
		}
	}
}
...
//Ryan april 2 2004 6:20pm
vmCvar_t	ui_admPunishCmd;
vmCvar_t	ui_clientKickReason;
vmCvar_t	ui_authMessage;
vmCvar_t	ui_songCount;
vmCvar_t	ui_voteIndex;
//vmCvar_t	ui_musicFolder;
//Ryan

// GodOrDevil - 03.31.06 - 04:55pm #Cvars
vmCvar_t nv_mapEditorInput;
// End - 03.31.06 - 04:55pm
...
static cvarTable_t cvarTable[] = 
{
	...
	//Ryan april 2 2004 6:17pm
	//for use when the admin has selected to kick or ban a player
	//this will be set to 1 to show the reason text box field
	{ &ui_admPunishCmd, "ui_admPunishCmd", "0", CVAR_ROM | CVAR_INTERNAL | CVAR_TEMP},
	{ &ui_clientKickReason, "ui_clientKickReason", "", CVAR_ROM | CVAR_TEMP},
	{ &ui_authMessage, "ui_authMessage", "", CVAR_ROM | CVAR_TEMP},
	{ &ui_songCount, "ui_songCount", "0", CVAR_ROM | CVAR_INTERNAL | CVAR_TEMP},
	{ &ui_voteIndex, "ui_voteIndex", "0", CVAR_ROM | CVAR_INTERNAL | CVAR_TEMP},
//	{ &ui_musicFolder, "ui_musicFolder", "RPM_Music", CVAR_ARCHIVE},
	//Ryan
	
// GodOrDevil #Cvars - UI
	{ &nv_mapEditorInput,	"nv_mapEditorInput",	"", CVAR_ROM | CVAR_INTERNAL},
///END
