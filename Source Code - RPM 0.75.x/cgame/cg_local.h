...
// GodOrDevil - 1.19.2005 - #ChatSmilies
////Ryan
////use these to store where and what smilie is to be drawn in the chat messages
typedef struct smilie_s
{
	float		x;
	qhandle_t	icon;
} smilie_t;

typedef struct chatLineIcon_s
{
	int			numSmilies;
	smilie_t	smilie[MAX_LINE_ICONS];
} chatLineIcon_t;
//End
...
typedef struct
{
	...
	//Ryan
	int				weapon;
	int				mute;			 //CJJ - 1.4.2005 - Support for display of MUTED status on clientside
	int				clanMember;		 // GodOrDevil - 1/06/2005 - Support for display of CLAN status on the clientside	
	int				admin;
	int				referee;
	char			cleanName[MAX_QPATH]; // copy of the players name clean of any colors
	//Ryan
	int				handicap;
	int				wins;
	int				losses;
...
typedef struct
{
	vec3_t				camOrigin;		// freecams origin
	vec3_t				camView;		// freecams origin
	vec3_t				camVel;
	qboolean			camFree;		// enable freecam movement
	qboolean			camFollow;		// follow the players viewpoint
	qboolean			hideCamShake;		// camera shake for freecam 
	qboolean			camThird;
	qboolean			draw2d;
	//qboolean			drawDemoInfo;
	//qboolean			drawEnts;
	//qboolean			drawWorld;

	float				camSpeed;		// Starting speed of the freecam
	int					camCount;
	int					camCurrent;
	int					camType;
	int					goggles;
...
//	snapshot_t		activeSnapshots[2];
	...
	//Ryan
	int				scoreNum;									// easier to refrence our score
	//Ryan
	...
	int					dnTime[MAX_CLIENTS];
	vec3_t				dnVecs[MAX_CLIENTS];
...
typedef struct
{
	...
	// GodOrDevil - 01/02/2006 #TeamIcons #Sprites
	qhandle_t   adminSpecShader;
	///End
	...
	sfxHandle_t rifleShot;
	qhandle_t	rpmIconShader;
	qhandle_t	rpm40mmShader;
	qhandle_t	blackShader;
	// GodOrDevil - 1.19.2005 - #ChatSmilies
	////Ryan
	chatLineIcon_t	chatIcon[CHAT_HEIGHT];
	qhandle_t	happySmilie;	
	qhandle_t	bigSmilie;		
	qhandle_t	sadSmilie;			
	qhandle_t	angryRedSmilie;	
	qhandle_t	secretSmilie;		
	qhandle_t	nerdSmilie;		
	qhandle_t	coolSmilie;		
	qhandle_t	surprisedSmilie; 
	qhandle_t	tongueSmilie;	
	qhandle_t	embarrassedSmilie; 
	qhandle_t	disappointedSmilie; 
	qhandle_t	confusedSmilie;
	qhandle_t	middleFinger;
	qhandle_t	thumbsUp;
	qhandle_t	thumbsDown;
	//End
...
extern	vmCvar_t		cg_zoomWeaponChange;		//ZoomWeaponChange - 1.08.2005
extern	vmCvar_t		cg_shotgunCrosshairSize;	//shotgunCrosshairScale - 1.08.2005
extern	vmCvar_t		cg_drawShotgunCrosshair;	//drawShotgunCrosshair - 1.21.2005
extern	vmCvar_t		nv_drawDemoInfo;
extern  vmCvar_t		cg_drawFriendScale;
extern  vmCvar_t		cg_friendlyNameAlpha;
extern  vmCvar_t		cg_friendlyNameColor;
extern  vmCvar_t		nvc_demoCamSpeed;
...
//Ryan
#include "rpm_cglocal.h"
#include "./nv/nvc_demo.h"
//Ryan
