...
void CG_NewClientInfo( int clientNum ) 
{
	...
	// Ryan Dec 18 2004
	// clean the players name from colors for use in the awards
	RPM_ClientCleanName( v, newInfo.cleanName, MAX_QPATH);
	// Ryan
...
static void CG_PlayerFloatSprite( centity_t *cent, qhandle_t shader, int team ) 
{
	int				rf;
	refEntity_t		ent;
...
static void CG_PlayerSprites( centity_t *cent ) 
{
	...
	// GodOrDevil - 10.30.05 - 07:45am - #Sprites
	if ( (cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR) && cgs.clientinfo[cg.clientNum].admin >= 2 && !(cent->currentState.number == cg.snap->ps.clientNum)) {
		//if (cg_drawFriend.integer)
		//	if (cgs.gametypeData->respawnType == RT_NORMAL && cgs.gametypeData->teams == qfalse)
		//		CG_PlayerFloatSprite( cent, cgs.media.adminSpecShader );
		//	else
		...
		if (cg_drawFriend.integer)
		{
			...
			{
				if(cg_weapons[cent->pe.weapon].weaponIcon)
				{		
					CG_PlayerFloatSprite( cent, cg_weapons[cent->pe.weapon].weaponIcon );
				}	
				else
				{
					CG_PlayerFloatSprite( cent, team==TEAM_RED?cgs.media.redFriendShader:cgs.media.blueFriendShader );
			...
			{
				vec3_t vec;
				NV_CalcSimpleMuzzlePoint(cent->currentState.number, vec);
				vec[2] += 21;
				NV_AddDrawName(cent->currentState.clientNum, vec, qtrue);
			}
		}
		return;
	}
	///Elim & INF Ghosts
	if(cgs.gametypeData->respawnType == RT_NONE && cgs.clientinfo[cg.clientNum].admin >= 2 && 
		(cg.snap->ps.pm_flags & (PMF_FOLLOW|PMF_GHOST)) && !(cent->currentState.number == cg.snap->ps.clientNum))
	{				
		if (cg_drawFriend.integer) 
		{	
			if(cg_drawFriend.integer == 7)
...
static void CG_PlayerSprites( centity_t *cent ) 
{
	...
	// GodOrDevil - 10.30.05 - 07:45am - #Sprites #Admin spec "helper"
	if ( (cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR) 
		&& cgs.clientinfo[cg.clientNum].admin >= 3
		...
		) 
	{
		if (cg_drawFriend.integer)
		{
			if (cgs.gametypeData->teams == qfalse)
			{
				CG_PlayerFloatSprite( cent, cgs.media.adminSpecShader, 0 );
			}
			else if(cg_drawFriend.integer == 1)
			{
				CG_PlayerFloatSprite( cent,team==TEAM_RED?cgs.media.redFriendShader:cgs.media.blueFriendShader, 0);
			}
			//else if(cg_drawFriend.integer == 8 && cg_weapons[cent->pe.weapon].weaponIcon)
			//{
			//	CG_PlayerFloatSprite( cent, cg_weapons[cent->pe.weapon].weaponIcon, team );
			//}
			else
			{
				vec3_t vec;
	...
	{			
		...
		if (cg_drawFriend.integer) 
		{			
			if(cg_drawFriend.integer == 8 && cg_weapons[cent->pe.weapon].weaponIcon)
			{		
				CG_PlayerFloatSprite( cent, cg_weapons[cent->pe.weapon].weaponIcon, team );
			}	
			else
			if(cg_drawFriend.integer == 1)
			{	
				CG_PlayerFloatSprite( cent, team==TEAM_RED?cgs.media.redFriendShader:cgs.media.blueFriendShader, 0 );
			}	
			else
...
//	CG_PlayerSplash( cent );
	...
	// GodOrDevil - AdminWH
	if (adm_wh && (cg.demoPlayback || !(cent->currentState.number == cg.snap->ps.clientNum) 
		&& ((cgs.gametypeData->respawnType == RT_NONE && cg.snap->ps.pm_flags & (PMF_FOLLOW|PMF_GHOST)) 
		|| (cgs.clientinfo[cg.clientNum].team == TEAM_SPECTATOR))))
	{
		if ( cgs.clientinfo[cg.snap->ps.clientNum].team != cgs.clientinfo[cent->currentState.clientNum].team) 
			trap_R_AddLightToScene(cent->lerpOrigin, 5, 100, 100, 100 );
		player.renderfx |= RF_DEPTHHACK|RF_NO_FOG;
		}
	// END
