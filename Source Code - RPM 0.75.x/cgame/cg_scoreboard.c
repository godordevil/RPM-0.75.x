...
//Ryan may 16 2004
float	sb_statFontScale;
float	sb_statFontSize;
//Ryan
...
static int CG_TeamScoreboard( float x, float y, float w, team_t team ) 
{
	...
	for ( skipped = -1, count = 0, i = 0 ; i < cg.numScores && count < sb_maxClients ; i++ ) 
	{
		...
		//#Ryan may 15 2004 #Scoreboard
		// GodOrDevil - 1.21.2005 - 0.5 ,0.6, 0.7 compatible? #TEST
		//CG_DrawClientScore( x, y + SB_HEADER_HEIGHT + sb_lineHeight * count, w, score, color );
		////if(cgs.rpmServer >= RPM_VERSION && cg_rpmScoreboard.integer)
		if(cgs.rpmServer >= 0.5 && cg_rpmScoreboard.integer)

		{
			RPM_DrawClientScore( x, y + SB_HEADER_HEIGHT + sb_lineHeight * count, w, score, color );
		}
		else
		{
			CG_DrawClientScore( x, y + SB_HEADER_HEIGHT + sb_lineHeight * count, w, score, color );
	...
	if ( skipped != -1 &&  count < sb_maxClients )
	{
		...
		{
			RPM_DrawClientScore( x, y + SB_HEADER_HEIGHT + sb_lineHeight * count, w, &cg.scores[skipped], color );
		}
		else
		{
			CG_DrawClientScore( x, y + SB_HEADER_HEIGHT + sb_lineHeight * count, w, &cg.scores[skipped], color );
...
qboolean CG_DrawNormalScoreboard ( float y )
{
	...
	if(cgs.rpmServer >= 0.5 && cg_rpmScoreboard.integer)
	{
		if ( ui_info_freecount.integer > 10 ) 
		{
			sb_maxClients	   = 16;
			sb_lineHeight	   = 20;
			sb_nameFontScale   = 0.25f;
			sb_readyFontScale  = 0.30f;
			sb_numberFontScale = trap_R_GetTextHeight ( "W", cgs.media.hudFont, sb_nameFontScale + 0.1f, 0 );
			sb_statFontScale   = trap_R_GetTextHeight ( "W", cgs.media.hudFont, sb_nameFontScale, 0 );
			sb_statFontSize	   = 0.22f;
		} 
		else
		{
			sb_maxClients	   = 10;
			sb_lineHeight	   = 30;
			sb_nameFontScale   = 0.33f;
			sb_readyFontScale  = 0.30f;
			sb_numberFontScale = trap_R_GetTextHeight ( "W", cgs.media.hudFont, sb_nameFontScale + 0.1f, 0 ) + 4;
			sb_statFontScale   = trap_R_GetTextHeight ( "W", cgs.media.hudFont, sb_nameFontScale, 0 ) + 2;
			sb_statFontSize	   = 0.26f;
		}
	}
	else
	{
...
qboolean CG_DrawTeamScoreboard( float y ) 
{
	...
	// GodOrDevil - Uncommented all this - #DEBUG - CHANGE IT BACK FOR A #TEST
/*
	if ( ui_info_redcount.integer > 10 || ui_info_bluecount.integer > 10 ) 
	{
		sb_maxClients	   = 16;
...
qboolean CG_DrawScoreboard ( void )
{
	...
	if ( (cg.warmup || cg.demoPlayback) && !cg.showScores) 
	{
		return qfalse;
	}
