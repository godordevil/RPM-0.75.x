...
static void CG_ParseScores( void )
{
	...
	{
		//#Ryan 14 2004 - #Scoreboard
		// stay backwardly compatible
	...
	*/
		//GodOrDevil - #Scoreboard #FIX
		if(cgs.rpmServer >= 0.6) //works with .7/.6
		////if(cgs.rpmServer >= 0.5)
		{
			cg.scores[i].client = atoi( CG_Argv( i * 11 + 4 ) );
			cg.scores[i].score = atoi( CG_Argv( i * 11 + 5 ) );
			cg.scores[i].kills = atoi( CG_Argv( i * 11 + 6 ) );
			cg.scores[i].deaths = atoi( CG_Argv( i * 11 + 7 ) );
			cg.scores[i].ping = atoi( CG_Argv( i * 11 + 8 ) );
			cg.scores[i].time = atoi( CG_Argv( i * 11 + 9 ) );
			cgs.clientinfo[ cg.scores[i].client ].ghost = atoi( CG_Argv( i * 11 + 10 ) );
			cgs.clientinfo[ cg.scores[i].client ].gametypeitems = atoi( CG_Argv( i * 11 + 11 ) );
			cg.scores[i].teamkillDamage = atoi( CG_Argv( i * 11 + 12 ) );
			cg.scores[i].accuracy = atof( CG_Argv( i * 11 + 13 ) );
			cg.scores[i].headshots = atoi( CG_Argv( i * 11 + 14 ) );
		/*	cg.scores[i].damageDone = atoi( CG_Argv( i * 18 + 15 ) );
			cg.scores[i].damageTaken = atoi( CG_Argv( i * 18 + 16 ) );
			cg.scores[i].ratio = atof( CG_Argv( i * 18 + 17 ) );
			cg.scores[i].shotsFired = atoi( CG_Argv( i * 18 + 18 ) );
			cg.scores[i].shotsHit = atoi( CG_Argv( i * 18 + 19 ) );
			cg.scores[i].explosiveKills = atoi( CG_Argv( i * 18 + 20 ) );
			cg.scores[i].knifeKills = atoi( CG_Argv( i * 18 + 21 ) );*/
		...
		//CJJ - 1.04.2005 - Spec ID  - //GodOrDevil - Changed Format a little
		////strcat ( cg.scoreBoardSpectators, va("%s (%d)", cgs.clientinfo[cg.scores[i].client].name, cg.scores[i].ping ) );
			strcat ( cg.scoreBoardSpectators, va("[%i]^-%s ^7(%d)", cg.scores[i].client, cgs.clientinfo[cg.scores[i].client].name, cg.scores[i].ping ) );
		}
...
static void CG_ConfigStringModified( void )
{
	...
	// GodOrDevil - 04.03.07 - 01:42am 
	else if ( num >= CS_BSP_MODELS && num < CS_BSP_MODELS+MAX_SUB_BSP ) 
	{
		NV_loadBSP(str);
	}
	// END
...
static void CG_AddChatText ( int client, const char *str )
{
	...
	//GodOrDevil - 1.19.2005 - #ChatSmilies
	//#Ryan Nov 28 2004, for simplicity #ChatIcons
	// points to the struct that holds the smilie info for 
	// the chat message line we are adding
	icon = &cgs.media.chatIcon[cgs.chatPos % chatHeight];
	//End
	len = 0;
	...
	*p = 0;
	...
	w  = 0;
	//GodOrDevil - 1.19.2005 - #ChatSmilies
	//#Ryan Nov 28 2004 #ChatIcons
	// Do some cleanup, we are going to overwrite this chat position now 
	// so clear any smilies
	if (cgs.chatPos - cgs.chatLastPos == chatHeight)
	{
		cgs.media.chatIcon[cgs.chatPos % chatHeight].numSmilies = 0;
	}
	//End
	while (*str)
	{
		...
		if ( w > 560 )
		{
			...
			*p = 0;
			...
			//GodOrDevil - 1.19.2005 - #ChatSmilies
			//#Ryan nov 28 2004 #ChatIcons
			// change the pointer to the struct for the next line
			icon = &cgs.media.chatIcon[cgs.chatPos % chatHeight];
			//End
		...
		//GodOrDevil - 1.19.2005 - #ChatSmilies
		//#Ryan Nov 28 2004 #ChatIcons
		// Look for smilie escape chars
		if(icon->numSmilies < MAX_LINE_ICONS)
		{
			numSkipChars = 0;

			if(*str == ':' && str[1] == ')') // :)
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.happySmilie;
				numSkipChars = 2;
			}
			else if(*str == ':' && (str[1] == 'd' || str[1] == 'D') ) // :d
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.bigSmilie;
				numSkipChars = 2;
			}
			else if(*str == ':' && str[1] == '(') // :(
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.sadSmilie;
				numSkipChars = 2;
			}
			else if(*str == ':' && str[1] == '@') // :@
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.angryRedSmilie;
				numSkipChars = 2;
			}
			else if(*str == ':' && str[1] == '-' && str[2] == '#') // :-#
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.secretSmilie;
				numSkipChars = 3;
			}
			else if(*str == '8' && str[1] == '-' && str[2] == '|') // 8-|
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.nerdSmilie;
				numSkipChars = 3;
			}
			else if(*str == '(' && str[1] == 'H' && str[2] == ')') // (H)
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.coolSmilie;
				numSkipChars = 3;
			}
			else if(*str == ':' && (str[1] == 'O' || str[1] == 'o'))// :O
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.surprisedSmilie;
				numSkipChars = 2;
			}
			else if(*str == ':' && (str[1] == 'P' || str[1] == 'p')) // :P
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.tongueSmilie;
				numSkipChars = 2;
			}
			else if(*str == ':' && str[1] == '$') // :$
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.embarrassedSmilie;
				numSkipChars = 2;
			}
			else if(*str == ':' && str[1] == '|') // :|
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.disappointedSmilie;
				numSkipChars = 2;
			}
			else if(*str == ':' && (str[1] == 'S' || str[1] == 's')) // :S
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.confusedSmilie;
				numSkipChars = 2;
			}
			else if(*str == ':' && (str[1] == 'F'|| str[1] == 'f')) // :F
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.middleFinger;
				numSkipChars = 2;
			}
			else if(*str == '(' && str[1] == 'Y'&& str[2] == ')') // (Y)
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.thumbsUp;
				numSkipChars = 3;
			}
			else if(*str == '(' && str[1] == 'N'&& str[2] == ')') // (N)
			{
				icon->smilie[icon->numSmilies].icon = cgs.media.thumbsDown;
				numSkipChars = 3;
			}
			// if we found a smilie escape
			if(numSkipChars)
			{
				// set this smilie x position relative to the start of the text
				*p = 0;
				icon->smilie[icon->numSmilies].x = (float)(trap_R_GetTextWidth( cgs.chatText[cgs.chatPos % chatHeight], cgs.media.hudFont, 0.38f, 0 ) + 1);
				icon->numSmilies++;

				// now skip the escape chars
				str += numSkipChars;
				len += numSkipChars;

				// and increase w accordingly
				w += (trap_R_GetTextWidth ( "  ", cgs.media.hudFont, 0.38f, 0 ));

				// make space for the smilie in the text by putting spaces
				*p++ = ' ';
				*p++ = ' ';
...
//	cgs.media.mAutomap = 0;			// make sure to re-upload the auto-map
	...
	//Ryan may 15 2004
	//clear all static values in the awards function
	RPM_DrawAwards(qtrue);
	//Ryan
...
*/
	...
	{	
		...
		{		
			...
			origin[0] = atoi(CG_Argv(2));
			origin[1] = atoi(CG_Argv(3));
			origin[2] = atoi(CG_Argv(4));
			s = CG_Argv(1);
			trap_FX_RegisterEffect (s);
			trap_FX_PlaySimpleEffect( s, origin, -1, -1);
		//	Com_Printf( " --- %s %.2f %.2f %.2f\n", s,  origin[0], origin[1], origin[2] );
		}
...
#endif
	//Ryan
	if( !strcmp( cmd, "headshot" ))
	{
		if(cg_headshotMessage.integer == 1)
		{
			const char* s;
		...
		if(cg_headshotMessage.integer == 2)
		{
			CG_CenterPrint( "^1HEADSHOT!", 0.85f );
		}

		if(cg_headshotSound.integer)
		{
			trap_S_StartLocalSound ( cgs.media.headshotSound, CHAN_AUTO );
		}
		return;
	}
	if (!strcmp(cmd, "tmi"))
