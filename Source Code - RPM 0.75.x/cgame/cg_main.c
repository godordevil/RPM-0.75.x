...
//GodOrDevil - 1.08.2005	- Cvars
vmCvar_t	cg_zoomWeaponChange;		//zoomWeaponChange - 1.08.2005
vmCvar_t	cg_shotgunCrosshairSize;	//shotgunCrosshairScale - 1.08.2005
vmCvar_t	cg_drawShotgunCrosshair;	//drawShotgunCrosshair	-	1.21.2005

vmCvar_t	nv_drawDemoInfo;

vmCvar_t	cg_drawFriendScale;
vmCvar_t	cg_friendlyNameAlpha;
vmCvar_t	cg_friendlyNameColor;
vmCvar_t	nvc_demoCamSpeed;
// END
...
vmCvar_t	cg_rpm;
vmCvar_t	cg_rpmClient;
vmCvar_t	cg_rpmHud;
vmCvar_t	cg_drawTeamMateInfo;
vmCvar_t	cg_rpmFirstRun;
vmCvar_t	cg_headshotMessage;
vmCvar_t	cg_headshotSound;
vmCvar_t	cg_rpmScoreboard;
vmCvar_t	cg_currentSong;
//Ryan
...
{ 
	...
	{ &cg_drawCrosshairNames, "cg_drawCrosshairNames", "2", CVAR_ARCHIVE },
	//Ryan
	...
	//GodOrDevil - 1.08.2005
	{ &cg_zoomWeaponChange,		"cg_zoomWeaponChange",		"2", CVAR_ARCHIVE,	0.0f,	0.0f },	//zoomWeaponChange - 1.08.2005
	{ &cg_shotgunCrosshairSize,	"cg_shotgunCrosshairSize",	"0", CVAR_ARCHIVE },				//shotgunCrosshairScale - 1.08.2005
	{ &cg_drawShotgunCrosshair, "cg_drawShotgunCrosshair",  "0", CVAR_ARCHIVE },				//lets you chose a different crosshair for shotty. 1.21.2005
	{ &nv_drawDemoInfo,			"nv_drawDemoInfo",			"1",		CVAR_ROM|CVAR_INTERNAL },
	{ &cg_drawFriendScale,		"cg_drawFriendScale",		"28",		CVAR_ARCHIVE, 10.0f, 100.0f  },
	{ &cg_friendlyNameAlpha,		"cg_friendlyNameAlpha",		".65",		CVAR_ARCHIVE, 0.01f, 1.0f  },
	{ &cg_friendlyNameColor,		"cg_friendlyNameColor",		"0",			CVAR_ARCHIVE, 0.1f, 100.0f  },
	{ &nvc_demoCamSpeed,		"nvc_demoCamSpeed",			"450",			CVAR_ARCHIVE, 0.1f, 1200.0f  },
	//End

	//Ryan
	{ &cg_rpmClient, "cg_rpmClient", RPM_VERSION_STRING, CVAR_ROM|CVAR_USERINFO  },
	{ &cg_drawTime, "cg_drawTime", "1", CVAR_ARCHIVE  },
	{ &cg_rpmHud, "cg_rpmHud", "1", CVAR_ARCHIVE  },
	{ &cg_drawTeamMateInfo, "cg_drawTeamMateInfo", "1", CVAR_ARCHIVE  },
	{ &cg_rpmFirstRun, "cg_rpmFirstRun", "1", CVAR_NORESTART|CVAR_ROM|CVAR_ARCHIVE },
	{ &cg_headshotMessage, "cg_headshotMessage", "2", CVAR_ARCHIVE  },
	{ &cg_headshotSound, "cg_headshotSound", "1", CVAR_ARCHIVE  },
	{ &cg_rpmScoreboard, "cg_rpmScoreboard", "1", CVAR_ARCHIVE  },
	{ &cg_currentSong, "cg_currentSong", "", CVAR_INTERNAL|CVAR_ROM},
	
	{ &cl_maxpackets, "cl_maxpackets", "125", CVAR_ARCHIVE|CVAR_LOCK_RANGE,  125, 125},
...
static void CG_RegisterSounds( void ) 
{
	...
	//Ryan
	cgs.media.headshotSound = trap_S_RegisterSound( "sound/headshot.wav");	
	cgs.media.rifleShot = trap_S_RegisterSound( "sound/misc/events/rifle_shot.mp3");
	//Ryan
...
static void CG_RegisterGraphics( void ) 
{
	...
	cgs.media.scoreboardLine   = trap_R_RegisterShaderNoMip ( "gfx/menus/scoreboard/scoreline2.png" );
	...
	{
		// GodOrDevil - 01/02/2006 - #TEAMICON #SPRITES
	...
	// GodOrDevil - 01/02/2006 - #TEAMICON #SPRITES
		...
		cgs.media.adminSpecShader = trap_R_RegisterShaderNoMip ( "gfx/menus/hud/team_free" );
	// End
...
// GodOrDevil - 03.27.07 - 03:08am #bsp - keep a count of bsp instances
		cgs.bspCount++;
// End  - 03.27.07 - 03:09am
...
void CG_StartMusic( qboolean bForceStart ) 
{
	...
	//Ryan  see if they are listening to their tunes first
	if(*cg_currentSong.string)
	{
		trap_S_StartBackgroundTrack( cg_currentSong.string, cg_currentSong.string, qfalse );
		return;
	}
	//Ryan
	...
	//Ryan march 7 2004 8:36pm
	else 
	{
		trap_S_StartBackgroundTrack( "music/ctf_kam", "music/ctf_kam", !bForceStart );
	}
	//Ryan
}
...
void CG_LoadMenus ( const char *menuFile ) 
{
	...
	if ( !f ) 
	{
		...
		Com_Error( ERR_FATAL, va( S_COLOR_RED "hud menu file not found: %s, unable to continue!\n", menuFile ) );
		//Ryan
		return;
	}
...
void CG_Init( int serverMessageNum, int serverCommandSequence, int clientNum ) 
{
	...
	//Ryan
	//cgs.media.rpmIconShader = trap_R_RegisterShaderNoMip ( "gfx/menus/icons/icon_modify_glow.png" );
	cgs.media.rpmIconShader = trap_R_RegisterShaderNoMip ( "gfx/menus/icons/RPM_logo.png" );
	cgs.media.rpm40mmShader = trap_R_RegisterShaderNoMip ( "gfx/menus/weapons/RPM_40mm.png" );
	cgs.media.blackShader = trap_R_RegisterShaderNoMip ( "gfx/colors/black" );

	//GodOrDevil - 1.19.2005 - #ChatSmilies	
	////Ryan
	//// load the chat icon grafix
	cgs.media.happySmilie	= trap_R_RegisterShaderNoMip ( "gfx/smilies/smile.png" );
	cgs.media.bigSmilie		= trap_R_RegisterShaderNoMip ( "gfx/smilies/teeth_smile.png" );
	cgs.media.sadSmilie		= trap_R_RegisterShaderNoMip ( "gfx/smilies/sad_smile.png" );
	cgs.media.angryRedSmilie = trap_R_RegisterShaderNoMip ( "gfx/smilies/angry_red.png" );
	cgs.media.secretSmilie	= trap_R_RegisterShaderNoMip ( "gfx/smilies/secret.png" );
	cgs.media.nerdSmilie	= trap_R_RegisterShaderNoMip ( "gfx/smilies/nerd.png" );
	cgs.media.coolSmilie	= trap_R_RegisterShaderNoMip ( "gfx/smilies/shades.png" );
	cgs.media.surprisedSmilie = trap_R_RegisterShaderNoMip ( "gfx/smilies/omg_smile.png" );
	cgs.media.tongueSmilie	= trap_R_RegisterShaderNoMip ( "gfx/smilies/tongue.png" );
	cgs.media.embarrassedSmilie	= trap_R_RegisterShaderNoMip ( "gfx/smilies/embarrassed.png" );
	cgs.media.disappointedSmilie = trap_R_RegisterShaderNoMip ( "gfx/smilies/what_smile.png" );
	cgs.media.confusedSmilie = trap_R_RegisterShaderNoMip ( "gfx/smilies/confused_smile.png" );
	cgs.media.middleFinger	= trap_R_RegisterShaderNoMip ( "gfx/smilies/finger.png" );
	cgs.media.thumbsUp		= trap_R_RegisterShaderNoMip ( "gfx/smilies/thumbs_up.png" );
	cgs.media.thumbsDown	= trap_R_RegisterShaderNoMip ( "gfx/smilies/thumbs_down.png" );
	//End

	//score
	award[0].text  = "Best Overall"; 
	award[0].medal = trap_R_RegisterShaderNoMip ( "gfx/mvpAward" );
	//headshots
	award[1].text  = "Head Hunter"; 
	award[1].medal = trap_R_RegisterShaderNoMip ( "gfx/headshotAward" );
	//damage
	award[2].text  = "Killer"; 
	award[2].medal = trap_R_RegisterShaderNoMip ( "gfx/damageAward" );
	//accuracy
	award[3].text  = "Accurate"; 
	award[3].medal = trap_R_RegisterShaderNoMip ( "gfx/accuracyAward" );
	//ratio
	award[4].text  = "Untouchable"; 
	award[4].medal = trap_R_RegisterShaderNoMip ( "gfx/ratioAward" );
	//explosives
	award[5].text  = "Explosives"; 
	award[5].medal = trap_R_RegisterShaderNoMip ( "gfx/explosivesAward" );
	//knives
	award[6].text  = "Knife Handling"; 
	award[6].medal = trap_R_RegisterShaderNoMip ( "gfx/knifeAward" );
	//Ryan
