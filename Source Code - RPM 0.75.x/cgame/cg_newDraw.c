...
qboolean CG_OwnerDrawVisible(int flags, const char* param ) 
{
	...
	while ( visible )
	{
		...
		// GodOrDevil
		#ifdef _DEMO
			//if(cg.demoCam.camFree && cg.demoPlayback)
			if(cg.demoPlayback)
			{
				if(cg.demoCam.goggles == DEMO_GOGGLES_OFF) // forced off
					visible = qfalse;
				else if(cg.demoCam.goggles == DEMO_GOGGLES_NV) // forced on
					visible = qtrue;
				else // whatever the user has
		...
		// GodOrDevil
			...
			if(cg.demoPlayback)
			{
				...
				else if(cg.demoCam.goggles == DEMO_GOGGLES_THERMALS) // forced on
				{
					visible = qtrue;
					 cg.predictedPlayerState.pm_flags &= PMF_GOGGLES_ON;
				}
				else
...
void CG_KeyEvent(int key, qboolean down) 
{
	...
	{
#ifdef _DEMO
		if(cg.demoPlayback && K_ESCAPE == key)
		{
			static int last = 0;
			static qboolean menuUp = qfalse;
			if(last == 0)
				last = cg.time;
			else if( last >= cg.time + 500)
				return;
			last = cg.time;
			if(!menuUp)
			//if (cg_timescale.integer == 1)
			{
