...
static void CG_DrawTeamScores ( float y )
{
	...
	//Dragon Dec 3 2003
	if ( cg_drawTeamScores.integer > 0 && cg_drawTeamScores.integer < 4 )
	{
...
static void CG_DrawCrosshair(void) 
{
	...
	//GodOrDevil - 1.21.2005 - #cg_drawShotgunCrosshair
	if (cg_drawShotgunCrosshair.integer && (cg.predictedPlayerState.weapon == WP_M590_SHOTGUN) || (cg.predictedPlayerState.weapon == WP_USAS_12_SHOTGUN))
	{
		ca = cg_drawShotgunCrosshair.integer;
		w = h = cg_shotgunCrosshairSize.value * 2;
	}
	else 
	{
	...
	// GodOrDevil - move above for shotgun check
	//ca = cg_drawCrosshair.integer;
	// END
...
static void CG_ScanForCrosshairEntity( void ) 
{
	...
	if ( cgs.gametypeData->teams )
	{
		if ( cg.predictedPlayerState.pm_type != PM_SPECTATOR && !(cg.predictedPlayerState.pm_flags&PMF_GHOST) && !(cg.predictedPlayerState.pm_flags&PMF_FOLLOW) )
		{
			...
			{
				///GodOrDevil - 11.03.06 - 11:24pm
				cg.crosshairColorClientNum = trace.entityNum;
				// End - 11.03.06 - 11:24pm
				return;
			}
		}
	}
...
static void CG_DrawCrosshairNames( void ) 
{
	...
	//Ryan
	if(cg_drawTeamMateInfo.integer && cg_drawCrosshairNames.integer == 1)
	{
		name = va("%s ^2[H^7%d%%^2] / ^2[A^7%d%%^2]",cgs.clientinfo[ cg.crosshairClientNum ].name, cgs.clientinfo[ cg.crosshairClientNum ].health, cgs.clientinfo[ cg.crosshairClientNum ].armor);
	}
	else
	{
	...
	else
	{
		...
		{
			y = 446;
			y -= trap_R_GetTextHeight(name, cgs.media.hudFont, 0.43f, 0);
		}
		else
		{
			y = 465;
	...
	//Ryan dec 10 2003
	if (cg_drawTeamMateInfo.integer && cg_drawCrosshairNames.integer == 2)
	{
		y += trap_R_GetTextHeight(name, cgs.media.hudFont, 0.43f, 0) + 2;
		name = va("^2[H^7%d%%^2] / ^2[A^7%d%%^2]", cgs.clientinfo[ cg.crosshairClientNum ].health, cgs.clientinfo[ cg.crosshairClientNum ].armor);
		w = trap_R_GetTextWidth(name, cgs.media.hudFont, 0.3f, 0);
		CG_DrawText( 320 - w / 2, y, cgs.media.hudFont, 0.3f, color, name, 0, 0 );
	}
	//Ryan
	trap_R_SetColor( NULL );
}
...
static void CG_DrawChat ( void ) 
{
	...
	if ( cg.time - cgs.chatTime[cgs.chatLastPos % chatHeight] > cg_chatTime.integer) 
	{
		...
		//// Do some cleanup, if not drawing this line anymore then remove its smilie count
		cgs.media.chatIcon[cgs.chatLastPos % chatHeight].numSmilies = 0;
	...
	for (i = cgs.chatPos - 1; i >= cgs.chatLastPos ; i--) 
	{
		...
		//GodOrDevil - 1.19.2005 - #ChatSmilies
		////#Ryan Nov 28 2004 #ChatIcons
		chatLineIcon_t *icon = &cgs.media.chatIcon[i % chatHeight];

		CG_DrawText ( x, y - (cgs.chatPos - i - 1) * 15,
					  font, scale, colorWhite, cgs.chatText[i % chatHeight], 0, DT_OUTLINE );
		if(icon->numSmilies) {
			int j = icon->numSmilies - 1;
			for(; j >= 0; j--)	{
				CG_DrawPic ( x + icon->smilie[j].x, y - ((cgs.chatPos - i - 1) * 15) + 2, 10, 10, icon->smilie[j].icon );
			}
		}
		//End
	}
...
static void CG_Draw2D( void ) 
{
	...
	if(cg.demoPlayback )
	{
		...
		{	
			...
			if(!cg.demoCam.draw2d)
			{	
				cg.scoreBoardShowing = CG_DrawScoreboard();
				if(!cg.scoreBoardShowing)
				{
					CG_DrawCenterText();
					trap_Cvar_Set ( "con_draw", "1" );
				}
				else
				{
					trap_Cvar_Set ( "con_draw", "0" );
				}
				CG_DrawChat ();
			}
			if(nv_drawDemoInfo.integer && !cg.scoreBoardShowing)
			{
				NV_drawDemoInfo();
				trap_Cvar_Set ( "con_draw", "1" );
			}
	...
	//Ryan may 6 2004
	//This is so we can display the awards with nothing else on the screen
	if ( cg.intermissionStarted == 2) 
	{
		RPM_DrawAwards(qfalse);
		CG_DrawChat();
		return;
	}
	//Ryan
	...
	CG_DrawChat ( );
#ifdef _DEMO
	if(cg.demoPlayback && !cg.scoreBoardShowing)
	{
		if(nv_drawDemoInfo.integer)
			NV_drawDemoInfo();
	}
#endif
}
...
void CG_DrawActive( stereoFrame_t stereoView ) 
{
	...
	if( !cg.demoPlayback && cg.popupObjectives )
	{
	...
	{	
		...
		case DEMO_GOGGLES_NV:
			...
			trap_R_DrawVisualOverlay ( GOGGLES_NIGHTVISION, qtrue, parm1, parm2);
			trap_R_RenderScene( &cg.refdef );
			trap_R_DrawVisualOverlay( GOGGLES_NIGHTVISION, qfalse, parm1, parm2);
			break;	
		case DEMO_GOGGLES_THERMALS:
			parm1 = cgs.mIRSeeThrough;
			parm2 = cgs.mIRDist;
			trap_R_DrawVisualOverlay (GOGGLES_INFRARED, qtrue, parm1, parm2);
			trap_R_RenderScene( &cg.refdef );
			trap_R_DrawVisualOverlay( GOGGLES_INFRARED, qfalse, parm1, parm2);
			break;	
		}
	}
	else
