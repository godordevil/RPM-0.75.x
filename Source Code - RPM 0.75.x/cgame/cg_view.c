...
#ifdef _DEMO
		// GodOrDevil
		if  (cg.demoPlayback && (cg.demoCam.camFree || (cg.renderingThirdPerson && (cg_thirdPersonRange.value >= 100 ||  cg_thirdPersonRange.value <= 50))))
		{
			cg.predictedPlayerState.pm_flags &= ~PMF_ZOOMED;
			zoomFov = fov_x;
			cg.predictedPlayerState.zoomFov = 0;
		}
		else
#endif
		// END
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	else
	{
		...
		{
			//Ryan
			if ( cg.snap->ps.pm_flags & PMF_FIRSTPERSONSPEC )
			{
				cg.renderingThirdPerson = qfalse;
			}
			//Ryan
